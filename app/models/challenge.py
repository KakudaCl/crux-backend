...
class Challenge(Base):
    ...
    grade = Column(
        ...
        comment="課題のグレード（例：6級、5級、V0など）"
    )
    climb_type = Column(
        String(50), default="ボルダリング", comment="クライミングの種類"
    )
    ...
    def __repr__(self):
        return (
            f"<Challenge(id={self.id}, user_name='{self.user_name}', "
            f"grade='{self.grade}', success={self.success})>"
        )
